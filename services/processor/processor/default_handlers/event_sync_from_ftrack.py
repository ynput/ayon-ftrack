...
class SyncProcess:
    ...
    def get_ftrack_entity_by_id(self, entity_id):
        ...
        Args:
            entity_id (str): Id of ftrack entity.

        Returns:
            Union[ftrack_api.Entity, None]: Entity matching entity id or None
        ...
        if entity_id == self.ft_project_id:
            return self.ft_project

        if entity_id in self._ftrack_entities_by_id:
            return self._ftrack_entities_by_id.get(entity_id)

        entity = self.session.query(
            self.entities_query_by_id.format(self.ft_project_id, entity_id)
        ).first()
    ...
    def initial_event_processing(self):
        ...
        found_actions = list(self._found_actions)
        if (
            len(found_actions) == 1
        ...
        self._is_event_valid = auto_sync is True
    ...
    def process_event_data(self):
        ...
        if ftrack_ids:
            ftrack_entities = self.session.query(
            ...
            for entity in ftrack_entities:
                self._ftrack_entities_by_id[entity["id"]] = entity

        print(10 * "*")
        for action, infos in self.entities_by_action.items():
            a = "\n".join([json.dumps(info, indent=4) for info in infos.values()])
            print(f"*** {action}\n{a}")
        print(10 * "#")
        return
    ...
    def _process_removed(self):
        """Handle removed entities from ftrack."""

        removed_entities = self.entities_by_action["remove"]
        if not removed_entities:
            return
        self.log.debug(f"Processing removed entities: {removed_entities}")
        removable_ids = []
        recreate_ents = []
        removed_names = []
        for ftrack_id, removed in removed_entities.items():
...
class AutoSyncFromFtrack(BaseEventHandler):
    ...
    def avalon_cust_attrs(self):
        if self._avalon_cust_attrs is None:
            self._avalon_cust_attrs = get_custom_attr_configs(
    ...
    def create_entity_in_avalon(self, ftrack_ent, parent_avalon):
        ...
        if mongo_id_str != ftrack_ent["custom_attributes"][CUST_ATTR_KEY_SERVER_ID]:
            ftrack_ent["custom_attributes"][CUST_ATTR_KEY_SERVER_ID] = mongo_id_str
            try:
                self.process_session.commit()
            except Exception:
                self.process_session.rolback()
    ...
    def get_cust_attr_values(self, entity):
        ...
        hier_values = get_hierarchical_attributes_values(
